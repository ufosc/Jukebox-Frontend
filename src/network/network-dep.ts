...
export class NetworkDep {
  static instance: NetworkDep
  protected routes: typeof NetworkEndpoints
  ...
  private constructor() {
    this.env = REACT_ENV
    this.routes = NetworkEndpoints
    this.token = null
  }

  public static getInstance = (): NetworkDep => {
    if (!NetworkDep.instance) {
      NetworkDep.instance = new NetworkDep()
    }

    return NetworkDep.instance
  }
  public static setMocks = (mocks: NetworkDep['mocks']) => {
  ...
  public sendGetUserInfo = async (): Promise<IUserDetails> => {
    ...
    const res = await this.sendRequest(this.routes.user.info)
  ...
  public async sendGetClubInfo(clubId: number): Promise<IClub> {
    ...
    const res = await this.sendRequest(this.routes.club.get(clubId))
  ...
  public async sendGetSpotifyToken(
    ...
    const res = await this.sendRequest(
      this.routes.jukebox.getSpotifyAccount(jukeboxId),
    )
  ...
  public async sendGetNextTracks(jukeboxId: number): Promise<IQueuedTrack[]> {
    if (this.env === 'dev') {
      await sleep(1000)
      return getRandomSample(mockQueuedTracks)
    }
